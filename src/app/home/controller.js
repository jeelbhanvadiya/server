...
exports.homePage = async (req, res) => {
    try {
        ...
        res.status(200).json({ status: 200, message: "Home page successfully!", data: { serviceData, stockData, rawMaterialData, attendanceData, ac_data }, error: {} });
    } catch (err) {
...
exports.homePageCount = async (req, res) => {
    try {
        ...
        res.status(200).json({ status: 200, message: "Home page count successfully!", data: { sellStockData, deleteStockData }, error: {} });
    } catch (err) {
        res.status(500).send({ message: err.message || "Some error occurred while retrieving login." });
    }
};